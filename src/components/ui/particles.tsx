...
interface Circle {
  ...
  translateX: number
  translateY: number
  translateZ: number  // Add z translation
  size: number
  alpha: number
  targetAlpha: number
  dx: number
  dy: number
  dz: number  // Add z velocity
  magnetism: number
}
...
export const Particles: React.FC<ParticlesProps> = ({
  ...
  const circleParams = (): Circle => {
    ...
    const dz = (Math.random() - 0.5) * 0.1  // Random z velocity
  ...
  const drawCircle = (circle: Circle, update = false) => {
    if (context.current) {
      ...
      context.current.fillStyle = `rgba(${rgb.join(", ")}, ${alpha * scale})`
  ...
  const animate = () => {
    ...
    circles.current.forEach((circle: Circle, i: number) => {
      ...
      circle.z += circle.dz + vz  // Update z position

      // Wrap z position
      if (circle.z > 100) circle.z = -100
      if (circle.z < -100) circle.z = 100

      circle.translateX += (mouse.current.x / (staticity / circle.magnetism) - circle.translateX) / ease
      circle.translateY += (mouse.current.y / (staticity / circle.magnetism) - circle.translateY) / ease
      circle.translateZ += (-circle.translateZ) / ease  // Gradually return to center on z-axis

      drawCircle(circle, true)

      // Update boundary check to include z-axis
      if (
